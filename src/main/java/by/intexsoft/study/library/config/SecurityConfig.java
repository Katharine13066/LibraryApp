package by.intexsoft.study.library.config;

import by.intexsoft.study.library.security.JwtConfigurer;
             ...
             //           "/feedback/getFeedbackByBookId/{id}", "/bookhistory/getBookHistoryByBookId/{id}").permitAll()
               // .antMatchers(HttpMethod.PUT, "/books/take_book/{bookId}/{userId}",
                 //       "/books/return_book/{bookId}/{userId}",
                   //     "/feedback").permitAll()
            //    .antMatchers(HttpMethod.POST, "/feedback", "/auth/login").permitAll()
